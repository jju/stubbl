...
[[badgerclaws]][[catcher]][[gcx]][[gcxi]][[superstar]][[active]]

Klim had been playing in the shadow of [[aeson]], but with the legend's retirement at the end of the eleventh Green Cup team management was looking to develop Klim for the future. He came out of the campaign as the team leader in SPP, even though he sat out a number of must-win games (coaches didn't trust him while Aeson was actually available).
...
|------------------|-------------|---------------|--:|--:|--:|---:|---:|---:|----:|---:|---:|---:|----:|----:|
| Klim   | [Badger Claws](../teams/badgerclaws) | Catcher |    8 |    3 |    9 |   20 |   21 |    7 |    1 |    0 |    0 |    0 |    4 |   92 |


```
SELECT pl.name AS Player,  pl.f_tname AS Team, pl.f_pos_name AS Position,  sum(mp.won) AS W, sum(mp.draw) AS D, sum(mp.lost) AS L, sum(mp.played) AS GP, sum(mp.td) AS TD, sum(mp.cp) AS Cp, sum(mp.intcpt) AS "Int", sum(mp.bh) AS BH, sum(mp.si) AS SI, sum(mp.ki) AS Ki, sum(mp.mvp) AS MVP, sum(mp.spp) AS SPP FROM mv_players AS mp  JOIN players AS pl ON mp.f_pid = pl.player_id AND mp.f_tid = pl.owned_by_team_id  JOIN tours ON mp.f_trid = tours.tour_id AND mp.f_did = tours.f_did  WHERE mp.f_did = 1 AND pl.name = "Klim" GROUP BY pl.name, pl.f_tname, pl.f_pos_name ORDER BY SPP DESC limit 11;
```
