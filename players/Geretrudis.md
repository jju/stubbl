...
[[xenothrillers]][[blitzer]][[ucv]][[experienced]]

Geretrudis had been mostly a nonentity in their Orange Goblet campaigns, but made their name with a game-saving interception at the very end of the UBBL Challenge V championship match.
...
|------------------|-------------|---------------|--:|--:|--:|---:|---:|---:|----:|---:|---:|---:|----:|----:|
| Geretrudis | Xeno Thrillers | Wight    |    8 |    2 |   10 |   20 |    1 |    0 |    1 |    0 |    1 |    0 |    0 |    7 |

```
SELECT pl.name AS Player,  pl.f_tname AS Team, pl.f_pos_name AS Position,  sum(mp.won) AS W, sum(mp.draw) AS D, sum(mp.lost) AS L, sum(mp.played) AS GP, sum(mp.td) AS TD, sum(mp.cp) AS Cp, sum(mp.intcpt) AS "Int", sum(mp.bh) AS BH, sum(mp.si) AS SI, sum(mp.ki) AS Ki, sum(mp.mvp) AS MVP, sum(mp.spp) AS SPP FROM mv_players AS mp  JOIN players AS pl ON mp.f_pid = pl.player_id AND mp.f_tid = pl.owned_by_team_id  JOIN tours ON mp.f_trid = tours.tour_id AND mp.f_did = tours.f_did  WHERE mp.f_did = 1 AND pl.name = "Geretrudis" GROUP BY pl.name, pl.f_tname, pl.f_pos_name ORDER BY SPP DESC limit 11;
```
...
|--------|--------------|-----------------|--:|--:|--:|---:|---:|----:|---:|---:|---:|----:|----:|
| Geretrudis | Xeno Thrillers | Orange Goblet II  |    2 |    0 |    2 |    0 |    0 |    0 |    0 |    1 |    0 |    0 |    2 |
| Geretrudis | Xeno Thrillers | Orange Goblet III |    1 |    1 |    8 |    0 |    0 |    0 |    0 |    0 |    0 |    0 |    0 |
| Geretrudis | Xeno Thrillers | UBBL Challenge V  |    5 |    1 |    0 |    1 |    0 |    1 |    0 |    0 |    0 |    0 |    5 |

```
SELECT pl.name AS Player, pl.f_tname AS Team, tours.name AS Season, mp.won AS W, mp.draw AS D, mp.lost AS L, mp.td as TD, mp.cp AS Cp, mp.intcpt as "Int", mp.bh AS BH, mp.si AS SI, mp.ki AS Ki, mp.mvp AS MVP, mp.spp AS SPP FROM mv_players AS mp  JOIN players AS pl ON mp.f_pid = pl.player_id AND mp.f_tid = pl.owned_by_team_id  JOIN tours ON mp.f_trid = tours.tour_id AND mp.f_did = tours.f_did  WHERE pl.name = "Geretrudis" ORDER BY tours.tour_id ASC;
```
